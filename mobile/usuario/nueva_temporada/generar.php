...
$nombre_liga = utf8_encode($liga->getValor("nombre"));
	...
	header ("Location: ../cerrar_sesion.php");
            ...
            while($resultados = $consulta->fetch_array(MYSQLI_ASSOC)){//cargamos las divisones y precios
				...
				while($resultados2 = $consulta2->fetch_array(MYSQLI_ASSOC)){//obtenemos los datos por division					
					...
					$ganados_aux[$cont_aux] = obten_consultaUnCampo('session','COUNT(id_partido)','partido','ganador',$resultados2['id_equipo'],'','','','','','','');//ganados
				...
				for($i=0; $i<count($id_equipo_aux); $i++){
					for($j=$i+1; $j<count($id_equipo_aux); $j++){
						if($ganados_aux[$j] > $ganados_aux[$i]){//si el siguiente es mayor que el base, hago cambio
							...
							if( ($sets_favor_aux[$j] - $sets_contra_aux[$j]) > ($sets_favor_aux[$i] - $sets_contra_aux[$i]) ){
								//copio el base
								$aux_id = $id_equipo_aux[$i];
								$aux_ga = $ganados_aux[$i];
								$aux_sf = $sets_favor_aux[$i];
			...
			for($i=0; $i<count($id_division); $i++){//HACER MOVIMIENTOS
				if($num_division[$i] == 1 &&  $i == 0){//campeón de 1a division
					...
					$asunto = utf8_decode('Nueva temporada para la Liga de Padel <'.$nombre_liga.'>.');
					$mail->Subject = $asunto;
					$linksi = 'http://miligadepadel.manuelbdv.es/mobile/v/v/valida_respuesta.php?id='.genera_id_url(100,$bd.'S'.$id_equipo[$i].'F',13);
					$linkno = 'http://miligadepadel.manuelbdv.es/mobile/v/v/valida_respuesta.php?id='.genera_id_url(100,$bd.'N'.$id_equipo[$i].'F',13);
		...
		<a style="width:70px;background-color:#039;margin-left:7%;padding: 5px 7px 5px 7px;font-size:18px;color:#FFF; text-decoration:none;box-shadow:2px 2px 3px rgba(0,0,0,0.5);border:1px #017 solid;" href="'.$linkno.'" target="_blank"><b>No</b></a><br><br>';
					...
					//$mail->AddBCC('manuel.berdelva@gmail.com');//añadimos al jugador2
					$body = email_jugadorAdmin("<br>¡Enhorabuena sois los campeones de la División ".$num_division[$i]." de la Liga ".$nombre_liga."!<br>",$cuerpo);
					$mail->msgHTML($body);
					...
					if($cont_aux == 0){//primer ascenso campeon
						...
						$asunto = utf8_decode('Nueva temporada para la Liga de Padel <'.$nombre_liga.'>.');
						$mail->Subject = $asunto;
						$linksi = 'http://miligadepadel.manuelbdv.es/mobile/v/v/valida_respuesta.php?id='.genera_id_url(100,$bd.'S'.$id_equipo[$i+$movimientos].'F',13);
						$linkno = 'http://miligadepadel.manuelbdv.es/mobile/v/v/valida_respuesta.php?id='.genera_id_url(100,$bd.'N'.$id_equipo[$i+$movimientos].'F',13);
			...
			<a style="width:70px;background-color:#039;margin-left:7%;padding: 5px 7px 5px 7px;font-size:18px;color:#FFF; text-decoration:none;box-shadow:2px 2px 3px rgba(0,0,0,0.5);border:1px #017 solid;" href="'.$linkno.'" target="_blank"><b>No</b></a><br><br>';
						...
						$mail->AddBCC($email2);//añadimos al jugador2
						//$mail->AddBCC('manuel.berdelva@gmail.com');//añadimos al jugador2
						$body = email_jugadorAdmin("<br>¡Enhorabuena, Campeones habéis ganado el ascenso a la División ".$num_division[$i]." de la Liga ".$nombre_liga."!<br>",$cuerpo);
						$mail->msgHTML($body);
						$mail->AltBody = 'This is a plain-text message body';//Replace the plain text body with one created manually
						$mail->send();
						unset($mail);
			...
			<a style="width:70px;background-color:#039;margin-left:7%;padding: 5px 7px 5px 7px;font-size:18px;color:#FFF; text-decoration:none;box-shadow:2px 2px 3px rgba(0,0,0,0.5);border:1px #017 solid;" href="'.$linkno.'" target="_blank"><b>No</b></a><br><br>';
						...
						$mail->AddBCC($email2);//añadimos al jugador2
						//$mail->AddBCC('manuel.berdelva@gmail.com');//añadimos al jugador2
						$body = email_jugadorAdmin("<br>¡Enhorabuena, habéis ascendido a la División ".$num_division[$i]." de la Liga ".$nombre_liga."!<br>",$cuerpo);
						$mail->msgHTML($body);
						$mail->AltBody = 'This is a plain-text message body';//Replace the plain text body with one created manually
						$mail->send();
						unset($mail);
			...
			<a style="width:70px;background-color:#039;margin-left:7%;padding: 5px 7px 5px 7px;font-size:18px;color:#FFF; text-decoration:none;box-shadow:2px 2px 3px rgba(0,0,0,0.5);border:1px #017 solid;" href="'.$linkno.'" target="_blank"><b>No</b></a><br><br>';
						...
						$mail->AddBCC($email2);//añadimos al jugador2
						//$mail->AddBCC('manuel.berdelva@gmail.com');//añadimos al jugador2
						$body = email_jugadorAdmin("<br>¡Enhorabuena, habéis realizado una gran temporada en la División ".$num_division[$i]." de la Liga ".$nombre_liga."!<br>",$cuerpo);
						$mail->msgHTML($body);
						$mail->AltBody = 'This is a plain-text message body';//Replace the plain text body with one created manually
						$mail->send();
						unset($mail);
				...
				insertar_regNuevaTemporada($id_liga,$id_division[$i],$id_equipo[$i],$i,$precio[$i]);//GENERAR NUEVA TEMPORADA
