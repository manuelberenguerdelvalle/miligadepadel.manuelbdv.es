...
if(isset($_GET["tx"])){
////pago real		//http://miligadepadel.manuelbdv.es/web/usuario/pago/gestion_pago.php?tx=9YB08729TX536901X&st=Completed&amt=0.50&cc=EUR&cm=&item_number=87
//http://miligadepadel.manuelbdv.es/web/usuario/registro/pago.php?tx=4A196903XN640160K&st=Completed&amt=0%2e50&cc=EUR&cm=&item_number=Inscripcion%20-1-2016-05-04%2021%3a00%3a23
		 ...
		 */
		 if($_SESSION['st'] == 'Completed' && $_SESSION['tx'] != '' && $_SESSION['item_number'] != ''){//SI ES CORRECTO CONTINUO
			if(obten_consultaUnCampo('unicas_liga','COUNT(id_pago_admin)','pago_admin','transaccion',$_SESSION['tx'],'','','','','','','') == 0){
				...
				$asunto = utf8_decode('Pago de Inscripción en la Liga <'.$_SESSION['nombre'].' división '.$_SESSION['num_division'].'> realizado correctamente!');
				$mail->Subject = $asunto;
				/*if(obten_resDespuesIgual($datos_rec[26]) == $_GET["item_number"]){//AQUI ENTRA SI ES PAGO CON TARJETA
				...
				$asunto = utf8_decode('Pago recibido correctamente en tu liga <'.$_SESSION['nombre'].' división '.$_SESSION['num_division'].'>.');
